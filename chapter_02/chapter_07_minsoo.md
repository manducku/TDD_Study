...
>**SMOKE TEST**란 무엇인가?
>기초 안전성 테스트 라는 뜻으로 프로그램의 중요한 기능들이 잘 돌아가는지 확인하기 위해    
>소프트웨어를 빌드한 이후에 실행하는 테스트를 말한다.    
>목적 자체가 ```조심하지 않아서 고장난 어플리케이션을 불합격처리하는 것```이기 때문에    
>소프트웨어를 설치하고 테스트하는데에 시간을 낭비하지 않는다.
